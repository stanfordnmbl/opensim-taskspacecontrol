        ...
        Vector& controls) const
{
    ...
    Vector controlInput = _priorityLevels[0].generalizedForce();
    ...
    for (unsigned int iP = 1; iP < _numPriorityLevels; iP++)
    {
        // TODO in which direction do I do this matrix multiplication?
        NT = NT * _priorityLevels[iP - 1].nullspaceProjectionTranspose(s);
        Vector Gamma_iP = _priorityLevels[iP].generalizedForce(s);

        controlInput += NT * Gamma_iP;
    }
    ...
    //      a + x(b + x(c + xd))
    /*
    Vector controlInput(_numCoords);
    for (unsigned int iP = _numPriorityLevels - 1; iP > 0; iP--)
    {
        // TODO in which direction do I do this matrix multiplication?
        NT = _priorityLevels[iP - 1].nullspaceProjectionTranspose(s);
        Vector Gamma_iP = _priorityLevels[iP].generalizedForce(s);

        controlInput = NT * (Gamma_iP + controlInput);
    }
    // The highest-priority level doesn't get filtered by a nullspace
    // projection.
    controlInput = _priorityLevels[0].generalizedForce() + controlInput;
    */
    ...
    {
        Vector thisActuatorsControls(1, controlInput[iCoord]);
